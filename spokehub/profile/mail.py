...
USERENA_USE_PLAIN_TEMPLATE = getattr(
    settings, 'USERENA_USE_PLAIN_TEMPLATE', not USERENA_HTML_EMAIL)


def _get_message_plain(message_plain, message_html):
...
def send_mail(subject, message_plain, message_html, email_from, email_to,
    ...
    if not message_plain and not message_html:
        raise ValueError(_("Either message_plain or "
                           "message_html should be not None"))

    message_plain = _get_message_plain(message_plain, message_html)
