...
class UserenaManager(UserManager):
    ...
    def confirm_email(self, confirmation_key):
        ...
        if SHA1_RE.search(confirmation_key):
            try:
                ...
                userena.email_unconfirmed = ''
                userena.email_confirmation_key = ''
                userena.save(using=self._db)
                user.save(using=self._db)
    ...
    def _get_perm_object(self, model, user):
        if model == 'profile':
            return get_user_profile(user=user)
        return user

    def _check_perms_for_model(self):
        changed_permissions = []
    ...
    def _check_user_perms(self, user, changed_users, warnings):
        try:
        ...
        return self._check_assigned_permissions(user, user_profile,
                                                changed_users, warnings)
    ...
    def check_permissions(self):
        ...
        changed_permissions = self._check_perms_for_model()
        changed_users = []
        warnings = []
        ...
        for user in get_user_model().objects.exclude(
            ...
            changed_users, warnings = self._check_user_perms(
                user, changed_users, warnings)
...
class UserenaBaseProfileManager(models.Manager):
    ...
    def get_visible_profiles(self, user=None):
        ...
        For now keeps it simple by just applying the cases when a user
        is not active, a user has it's profile closed to everyone or a
        user only allows registered users to view their profile.
